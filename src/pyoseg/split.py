...
def validate_split(trainf, valf, testf, e=1e-5):
    ...
    Parameters:
        ...
        e (float, optional): A small epsilon value to account for floating
            point errors. Defaults to 1e-5.

    Raises:
        AssertionError: If the sum of the fractions is not approximately equal
            to 1.
    ...
    if not (fraction_sum < 1+e and fraction_sum > 1-e):
        raise ValueError(
            f"""Split fractions dont sum up to 1 (100%), please check \
            fractions: \nTrain: {trainf}\nValidation: {valf}\nTest: {testf}""")
...
def split_array(arr, size, vf, tf):
    ...
    if size > 2:
        ...
        if i_test != size:
            raise ValueError(
                "The sum over split sets is not the total number of elements.")
...
def create_annotations(
        ...
        random=False):
    """
    Creates annotations for a given input folder and writes them to an output
    file.

    Parameters:
        input_folder (str): The path to the folder containing the input files.
        output_file (str): The path to the output file where the annotations
            will be written.
        ids (list, optional): A list of specific ids to include in the
            annotations. Defaults to None.
        annotation_suffix (str, optional): The suffix for the annotation files.
            Defaults to "_coco.json".
        random (bool, optional): Whether to create in random order. Defaults to
            False.

    Returns:
        dict: The merged data containing the images, annotations, and
            categories.
    """
    merged_data = {
        "images": [],
    ...
    annotation_files = \
        [f for f in os.listdir(input_folder) if f.endswith(annotation_suffix)]
    if ids is not None:
        fann = []
    ...
    if ids is None or len(ids) != 0:
        if len(annotation_files) == 0:
            raise FileNotFoundError(
                f"No annotation files found in input folder {input_folder}.")
        if random:
...
def create_split_annotations(
    ...
    Returns:
        tuple: A tuple containing the split configuration and the merged
    ...
    train_path, val_path, test_path = \
        annotations_path, annotations_path, annotations_path
    if augmentation is not None:
        ids, paths = augment_ids(
            config, annotations_path, output_path,
            annotation_suffix, augmentation, shuffle=True)
        train_ids, val_ids, test_ids = \
            ids["train_ids"], ids["val_ids"], ids["test_ids"]
        train_path, val_path, test_path = \
            paths["train_ids"], paths["val_ids"], paths["test_ids"]

    # Create the merged annotation file for each set
    print(" > Split sizes:")
    print(f"Training split size:         {len(train_ids)}")
    train_ann = create_annotations(
        train_path, f"{output_path}/train_annotations.json",
        train_ids, annotation_suffix)
    print(f"Validation split size:       {len(val_ids)}")
    val_ann = create_annotations(
        val_path, f"{output_path}/validation_annotations.json",
        val_ids, annotation_suffix)
    print(f"Testing split size:          {len(test_ids)}")
    test_ann = create_annotations(
        test_path, f"{output_path}/test_annotations.json",
        test_ids, annotation_suffix)

    # Assert that split size and merged size are the same
    if len(train_ids) != len(train_ann['images']) or \
        len(val_ids) != len(val_ann['images']) or \
            len(test_ids) != len(test_ann['images']):
        raise ValueError(
            "Expected number of ids for training, validation and testing is \
            different than what was obtained while merging annotation files.")
...
def augment_ids(
    ...
    Parameters:
        split (dict): A dictionary containing the splits as keys and a list of
            file names as values.
        annotations_path (str): The path to the directory containing the
            annotations.
        output_path (str): The path to the directory where the augmented
            images will be saved.
        annotation_suffix (str): The suffix of the annotation files.
        augmentation (dict): A dictionary containing the augmentation
            configuration.
        shuffle (bool, optional): Whether to shuffle the augmented image ids.
            Defaults to True.

    Returns:
        tuple: A tuple containing two dictionaries. The first dictionary
            contains the augmented image ids for each split, and the second
            dictionary contains the corresponding paths to the augmented
            images.
    """
    ...
    for key in split.keys():
        ...
        for file in split[key]:
            os.system(
                f"cp {dir_data}/{file}.png {output_path}/{key}/data")
            os.system(
                f"cp {dir_anns}/{file}_coco.json \
                    {output_path}/{key}/annotations")
        ...
        i_id = [f[:-len(annotation_suffix)] for f in os.listdir(
            f"{output_path}/{key}/augmented") if annotation_suffix in f]
        if shuffle:
...
def create_split(
    ...
    Parameters:
        annotations_path (str): The path to the directory containing the
            annotation files.
        output_path (str, optional): The output path for the split annotations.
            Defaults to "output".
        annotation_suffix (str, optional): The suffix of the annotation files.
            Defaults to "_coco.json".
        train_fraction (float, optional): The fraction of data to be included
            in the training set. Defaults to 0.7.
        validation_fraction (float, optional): The fraction of data to be
            included in the validation set. Defaults to 0.2.
        test_fraction (float, optional): The fraction of data to be included
            in the test set. Defaults to 0.1.
        augmentation (dict): Configuration on how to augment the data.
            Defaults to None.
        seed (int): The seed for the random number generator. Defaults to None.
        limit_train (int): Limit the number of images for training. Important
            for learning curve.
    ...
    if (TEST_MODE and size > 0) or (seed < 0):
        arr.sort()
    ...
    # Split the array
    train_ids, val_ids, test_ids = split_array(
        arr, size, validation_fraction, test_fraction)
    if limit_train > 0:
        train_ids = train_ids[0:limit_train]

    # Create the annotations based on the split
    return create_split_annotations(
        train_ids, val_ids, test_ids, annotations_path, output_path,
        annotation_suffix, augmentation)
...
def create_split_cluster(
    ...
    Parameters:
        cluster_file (str): Path to the clustering file (default is
            "Membrane_small_img_clusters.csv").
        cluster_column (str): Name of the column in the clustering file that
            contains the cluster labels (default is "PhenoGraph_clusters").
        image_column (str): Name of the column in the clustering file that
            contains the image filenames (default is "Images").
        annotations_path (str): Path to the directory containing the
            annotation images.
        output_path (str): Path to the directory where the split clusters will
            be saved (default is "output").
        annotation_suffix (str): Suffix of the annotation image files (default
            is "_coco.json").
        train_fraction (float): Fraction of the data to use for training
            (default is 0.7).
        validation_fraction (float): Fraction of the data to use for
            validation (default is 0.2).
        test_fraction (float): Fraction of the data to use for testing
            (default is 0.1).
        seed (int): The seed for the random number generator. Defaults to None.
        augmentation (str): The configuration on how to augment the data.
            Defaults to None.
        limit_train (int): Limit the number of images for training. Important
            for learning curve.
    ...
    # Get annotation file ids
    current_imgs = np.array(
        [f for f in os.listdir(annotations_path) if annotation_suffix in f])
    to_rm = len(annotation_suffix)
    ...
    for c in clusters:
        ...
        train_s, val_s, test_s = split_array(
            arr, size, validation_fraction, test_fraction)
    ...
    # Create the annotations based on the split
    return create_split_annotations(
        train_ids, val_ids, test_ids, annotations_path,
        output_path, annotation_suffix, augmentation)
